...
MIXING_SCHEDULE = [0.0, 0.2, 0.5, 0.7]  # Round 1 = 0.2, Round 2 = 0.5, etc.
...
def get_defender_obs(positions, velocities, ball_pos, ball_vel, team_idx, n):
    """16-feature observation. Handles Team B (defending left) and Team A (mirrored)."""
    player_idx = n if team_idx == 1 else 0

    # Logic: Team B (1) defends x=0. Team A (0) defends x=FIELD_W.
    OWN_GOAL = np.array([0.0, FIELD_H / 2]) if team_idx == 1 else np.array([FIELD_W, FIELD_H / 2])
    SCORING_GOAL = np.array([FIELD_W, FIELD_H / 2]) if team_idx == 1 else np.array([0.0, FIELD_H / 2])
    ...
    # Binary: Is ball in the half we are defending?
    if team_idx == 1:
        ball_in_own_half = 1.0 if ball_pos[0] < FIELD_W / 2 else 0.0
    else:
        ball_in_own_half = 1.0 if ball_pos[0] > FIELD_W / 2 else 0.0
    ...
    return np.array([
        ...
        np.clip(botg_dist / FIELD_W, 0, 1), np.clip(ball_to_sg_dist / FIELD_W, 0, 1),
        np.clip(ball_heading_danger, -1, 1), ball_in_own_half
    ], dtype=np.float32)


def get_attacker_obs(positions, velocities, ball_pos, ball_vel):
    """Original 12-feature observation for the frozen attacker model."""
    p = positions[0];
    vel = velocities[0]
    ...
    return np.array([
        p[0] / FIELD_W * 2 - 1, p[1] / FIELD_H * 2 - 1,
        vel[0], vel[1], to_ball[0] / FIELD_W, to_ball[1] / FIELD_H,
...
class DefenderEnv(gym.Env):
    def __init__(self, render_mode=None, self_play_opponent_path=None, frozen_fraction=0.0, total_steps_ref=None):
        super().__init__()
        ...
        # Load models
        self._cached_attacker = PPO.load(ATTACKER_PATH) if os.path.exists(f"{ATTACKER_PATH}.zip") else None
    ...
    def reset(self, seed=None, options=None):
        ...
        self._use_selfplay_opp = (self._self_play_path and np.random.rand() < self._frozen_fraction)

        return get_defender_obs(self._env._pos, self._env._vel, self._env._ball_pos, self._env._ball_vel, 1, 1), info

    def step(self, action_b):
        # 1. Select Opponent and matching Observation Shape
        if self._use_selfplay_opp:
            if self._cached_selfplay is None:
                self._cached_selfplay = PPO.load(self._self_play_path)
            # MIRRORED DEFENDER (16 features)
            opp_obs = get_defender_obs(self._env._pos, self._env._vel, self._env._ball_pos, self._env._ball_vel, 0, 1)
            opp_model = self._cached_selfplay
        else:
            # ORIGINAL ATTACKER (12 features)
            opp_obs = get_attacker_obs(self._env._pos, self._env._vel, self._env._ball_pos, self._env._ball_vel)
        ...
        obs_dict, env_rewards, terminated, truncated, info = self._env.step({
            "team_a": action_a.reshape(1, 2), "team_b": np.array(action_b).reshape(1, 2)
        })

        if self._total_steps_ref: self._total_steps_ref[0] += 1

        # 3. Reward Shaping
        ball_pos, player_pos = info["ball_pos"], self._env._pos[1]
        ...
        if self._counter_state == "counter":
            ...
            if att_dist < (PLAYER_RADIUS + BALL_RADIUS + 0.8) or self._steps_in_counter > MAX_COUNTER_STEPS:
                self._counter_state = "defend"

        r_goal = env_rewards["team_b"] * 10.0
        r_counter_bonus = 10.0 if env_rewards["team_b"] > 0 and self._ball_touched_by_def else 0.0
        ...
        return get_defender_obs(self._env._pos, self._env._vel, ball_pos, self._env._ball_vel, 1,
                                1), reward, terminated, truncated, info
...
class TrainingCallback(BaseCallback):
    def __init__(self, total_steps, round_name):
        ...
        self.total_steps, self.round_name, self.best_rew, self._last_stage = total_steps, round_name, -float("inf"), -1
    ...
    def _on_rollout_end(self):
        ...
        stage = next((i for i, (th, _, _) in enumerate(ANNEAL_STAGES) if progress <= th), len(ANNEAL_STAGES) - 1)
...
if __name__ == "__main__":
    steps_ref = [1_500_000]  # Set to end of Phase 0 for correct annealing
    ...
    for r in range(1, SELF_PLAY_ROUNDS + 1):
        ...
        env = SubprocVecEnv([lambda: Monitor(DefenderEnv(self_play_opponent_path=SELF_PLAY_PATH, frozen_fraction=frac,
                                                         total_steps_ref=steps_ref))] * N_ENVS)

        model = PPO.load(DEFENDER_NAME, env=env)
        model.learn(total_timesteps=STEPS_ROUND, callback=TrainingCallback(STEPS_ROUND, f"round_{r}"),
                    progress_bar=True)
