...
def _make_obs(positions, velocities, ball_pos, ball_vel, team_idx, n):
    # flat vector: own n players (x,y,vx,vy), opp n players (x,y,vx,vy), ball (x,y,vx,vy) — all in [-1,1]
    own = list(range(n))       if team_idx == 0 else list(range(n, 2 * n))
    opp = list(range(n, 2*n))  if team_idx == 0 else list(range(n))

    def norm_p(p):
        return np.array([p[0] / FIELD_W * 2 - 1, p[1] / FIELD_H * 2 - 1], dtype=np.float32)

    def norm_v(v):
        return np.clip(np.array([v[0] / MAX_SPEED, v[1] / MAX_SPEED], dtype=np.float32), -1, 1)
    ...
    parts.append(np.clip(
        np.array([ball_vel[0] / KICK_POWER, ball_vel[1] / KICK_POWER], dtype=np.float32), -1, 1
    ))
...
class SoccerEnv(gym.Env):
    ...
    def __init__(self, render_mode=None, n_players=1):
        ...
        single_obs = spaces.Box(low=-1, high=1, shape=(obs_size,), dtype=np.float32)
        self.observation_space = spaces.Dict({"team_a": single_obs, "team_b": single_obs})
    ...
    def _reset_positions(self):
        ...
        for i in range(n):
            offset_y = (i - (n - 1) / 2) * 6
            self._pos[i]     = [cx - 5 - i * 3, cy + offset_y]
            self._pos[n + i] = [cx + 5 + i * 3, cy + offset_y]
        self._vel[:] = 0
        self._ball_pos[:] = [cx, cy]
        self._ball_vel[:] = 0

    def step(self, actions):
        n, N = self.n, 2 * self.n
        all_act = np.clip(np.concatenate([actions["team_a"], actions["team_b"]], axis=0), -1, 1)

        # --- move players (inertia smooths jitter) ---
        INERTIA = 0.75
        for i in range(N):
            move = all_act[i, :2]
            ...
            self._pos[i] = np.clip(self._pos[i] + self._vel[i],
        ...
        for i in range(N):
            ...
            if 1e-6 < d < md:
                self._ball_pos += diff / d * (md - d)

        # --- goal check: returns ±1 reward, resets positions ---
        ra, rb = 0.0, 0.0
    ...
    def _get_obs(self):
        return {
            "team_a": _make_obs(self._pos, self._vel, self._ball_pos, self._ball_vel, 0, self.n),
            "team_b": _make_obs(self._pos, self._vel, self._ball_pos, self._ball_vel, 1, self.n),
        }
    ...
    def render(self):
        ...
        if self._renderer is None:
            ...
            for (sx, sy, sw, sh) in strips:
                ...
                while y < sy + sh:
                    ...
                    while x < sx + sw:
                        pygame.draw.circle(self._crowd, FAN_COLORS[rng.integers(0, len(FAN_COLORS))], (x, y), DOT_R)
                        x += STEP
                    y += STEP
                    row += 1
        ...
        pa_w, pa_h = int(8 * SCALE), int(16 * SCALE)
        pa_y = MB + (H - pa_h) // 2
        ...
        surf.blit(stxt, (TW // 2 - stxt.get_width() // 2, MB // 2 - stxt.get_height() // 2))
