...
def get_relative_obs(positions, velocities, ball_pos, ball_vel, team_idx, n):
    ...
    parts = []
    # Goal target is the center of the net
    ...
    for i in own:
        ...
        parts += [
            p[0] / FIELD_W * 2 - 1, p[1] / FIELD_H * 2 - 1,  # Absolute Pos
            velocities[i][0], velocities[i][1],  # Velocity
            to_ball[0] / FIELD_W, to_ball[1] / FIELD_H,  # Vector to Ball
            to_goal[0] / FIELD_W, to_goal[1] / FIELD_H  # Vector to Goal
        ]
    # Ball info
    parts += [ball_pos[0] / FIELD_W * 2 - 1, ball_pos[1] / FIELD_H * 2 - 1, ball_vel[0], ball_vel[1]]
...
class TeamAEnv(gym.Env):
    ...
    def step(self, action_a):
        ...
        obs, env_rewards, terminated, truncated, info = self._env.step({
            "team_a": action_a_2d, "team_b": action_b.reshape(1, 2)
        })
        ...
        ball_pos, player_pos = info["ball_pos"], self._env._pos[0]
        ...
        if contact_edge:
            if ball_speed > 0.4:
                r_kick = 0.1  # Bonus for random contact that moves ball
                ...
                if direction_match > 0.4:
                    r_kick += 0.7  # Extra 0.7 ONLY if aimed at goal net (Anti-Wall)
            else:
                r_kick = 0.02  # Tiny touch reward

        # 4. PROGRESS (The Scent Trail)
        r_prog = 0.0
        if ball_speed > 0.1:
...
class TrainingCallback(BaseCallback):
    def __init__(self, total_steps, round_name):
        ...
        self.total_steps, self.round_name = total_steps, round_name
        self.best_rew, self._last_stage = -float('inf'), -1
...
def build_model(env):
    return PPO("MlpPolicy", env, verbose=1, n_steps=512, batch_size=256, n_epochs=10,
               learning_rate=1.5e-4, ent_coef=0.02, policy_kwargs={"net_arch": [256, 256]})
...
if __name__ == "__main__":
    ...
    for r in range(2, SELF_PLAY_ROUNDS + 1):
        ...
        env = SubprocVecEnv([lambda: Monitor(TeamAEnv(opponent_path=OPPONENT_PATH,
                                                      frozen_fraction=MIXING_SCHEDULE[r - 1],
                                                      total_steps_ref=steps_ref))] * N_ENVS)
        model = PPO.load(MODEL_NAME, env=env)
        model.learn(total_timesteps=STEPS_ROUND, callback=TrainingCallback(STEPS_ROUND, round_n), progress_bar=True)
        if os.path.exists(f"best_{round_n}.zip"): shutil.copy(f"best_{round_n}.zip", f"{MODEL_NAME}.zip")
        env.close()